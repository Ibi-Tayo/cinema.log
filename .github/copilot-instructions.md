...
npm run test  # Uses Karma + Jasmine
```
...
- Base API URL from [environments/environment.ts](cinema.log.client/src/environments/environment.ts) (`apiUrl`)
...
Frontend: Uses [environment.ts](cinema.log.client/src/environments/environment.ts) (prod) and `environment.development.ts` (dev)
...
- Production backend: Railway deployment (see `environment.apiUrl`)

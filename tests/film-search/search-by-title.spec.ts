...
test.describe('Film Search', () => {
  ...
  test('User can search for films by title', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Search for a film by title...' })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /Search Results \(\d+\)/ })).toBeVisible();
    await expect(page.getByRole('img', { name: 'The Dark Knight' }).first()).toBeVisible();
    await expect(page.getByRole('heading', { name: 'The Dark Knight Rises' }).first()).toBeVisible();
  });
});

...
test.describe('Film Review and Rating', () => {
  ...
  test('User can submit a complete film review', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Casablanca' })).toBeVisible();

    // 2. Check if this is a new review (star buttons visible) or an update
    const starButton = page.getByTestId('review-form-star-5');
    const isNewReview = await starButton.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    if (isNewReview) {
      ...
      await page.getByTestId('review-form-textarea').fill('A masterpiece of storytelling with unforgettable dialogue');
      ...
      await expect(page.getByRole('heading', { name: 'Update Review' })).toBeVisible();
    } else {
      // Update review flow - film has already been reviewed
      await expect(page.getByRole('heading', { name: 'Update Review' })).toBeVisible();
      
      // Update the review text
      await page.getByTestId('review-form-textarea').fill('Updated review: A timeless classic!');
      await page.getByRole('button', { name: 'Update Review' }).click();
      await expect(page.getByText(/Review (updated|submitted) successfully/i)).toBeVisible();
    }
  });
});

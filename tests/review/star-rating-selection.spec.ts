...
test.describe('Film Review and Rating', () => {
  ...
  test('User can select different star ratings', async ({ page }) => {
    ...
    await page.getByRole('img', { name: 'Schindler\'s List', exact: true }).click();
    await expect(page.getByRole('button', { name: 'Rate 1 out of 5 stars' })).toBeVisible();
    await expect(page.getByRole('button', { name: 'Rate 5 out of 5 stars' })).toBeVisible();

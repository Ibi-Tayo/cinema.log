...
test.describe('Film Review and Rating', () => {
  ...
  test('User cannot submit review without rating', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Review Film' })).toBeVisible();

    // 2. Type review text without selecting a star rating
    await page.getByTestId('review-form-textarea').fill('This is a great movie but I haven\'t rated it');

...
test.describe('Home Page', () => {
  ...
  test('Home page displays correct content', async ({ page }) => {
    ...
    await expect(page.getByText('A smarter way to track and rate the films you love')).toBeVisible();
    
    // expect: Three main content sections are visible: 'Review & Rate Films', 'ELO Rating System', and 'Ready to Start?'
    await expect(page.getByRole('heading', { name: 'Review & Rate Films' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'ELO Rating System' })).toBeVisible();
    ...
    await expect(page.getByText('Rate films from 1 to 5 stars based on your experience')).toBeVisible();
  });
});
